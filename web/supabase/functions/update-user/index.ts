...
Deno.serve(async (req) => {
  try {
    ...
    // Treat null/empty as "no change" for email/display_name.
    const emailRaw = body.email === undefined ? undefined : (body.email ?? '').trim().toLowerCase()
    const email = emailRaw === '' ? undefined : emailRaw

    const displayNameRaw = body.display_name === undefined ? undefined : (body.display_name ?? '').trim()
    const displayName = displayNameRaw === '' ? undefined : displayNameRaw

    const passwordRaw = body.password === undefined ? undefined : (body.password ?? '')
    ...
    return json(500, { error: 'Unhandled error', detail: message })
  }
})
